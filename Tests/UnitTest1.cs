...
namespace Tests
{
    ...
    public class UnitTest1
    {
        ...
        private void CreateAccount(Guid id)
        {
            ...
            _fixture.MainBus.TrySend(cmd, out var _, TimeSpan.FromMilliseconds(3000), TimeSpan.FromMilliseconds(3000)); //"Failed to create a new Account");
        ...
        private void LoadEvents(Guid id, int n)
        {
            for (var i = 0; i < n; i++)
            {
                ...
                if (!_fixture.MainBus.TrySend(cmd, out var _, TimeSpan.FromMilliseconds(3000), TimeSpan.FromMilliseconds(3000)))
